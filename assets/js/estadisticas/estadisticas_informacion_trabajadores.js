...
var _data_default_chart_desempeno = [ 100, 100, 100,100];
...
// Objeto para realizar la carga delos datos
	var cargar_datos = {
		cargar_coordinaciones : function(_mes, _ano){
			$.ajax({
				url: _base_url+'Estadisticas/ajax_informacion_general_trabajadores',
				type: 'POST',
				dataType: 'JSON',
				asyc : false,
			...
			.done(function(data) {
				// console.log("success");
				// console.log(data);	
				rellenar.tabla_trabajadores(data);
			})
			.fail(function() {
				console.log("error");
			})
...
// Objeto para realizar el lleano de los datos en la vista
	var rellenar = {
		tabla_trabajadores : function(data){
			...
			$.each(data, function(index, val) {
				...
				$("#tb_trabajadores > tbody").append(
					'<tr>'
						...
						+'<td class="text-center col-md-4">'+val.apellido_nombre+'</td>'
						+'<td>'+val.cargo+'</td>'
						+'<td>'+val.condicion_laboral+'</td>'
						// +'<td><span class="badge bg-blue col-md-1">'+val.coordinacion+'%</span></td>'
						// +'<td><span class="badge bg-yellow col-md-1">'+val.direccion+'%</span></td>'
						// +'<td><span class="badge bg-gray">'+val.por_horas_trabajadas_djornada+'%</span></td>'
						+'<td>'+val.nro_horas_trabajadas+'</td>'
